...
**Deadline:** Fri 25 Sep 2026, end of day (America/Lima or UTC; missing MUST items = partial kill).
**Live `main` (verified 2026-09-21):** `be90513` — `feat(doctor): fail on insecure defaults + honesty/CI (#15)`.
**Original inspection SHA:** `22ef1c2` (pre-#15). Do not treat the historical sections below as current code state.

Product lock: Ghost = OSS local self-audit case-file (CLI + SQLite + provenance + doctor). **NO SaaS. NO Express. NO Fairway.**
Env names locked: `GHOST_SECRET_KEY`, `GHOST_HOST`, `GHOST_API_TOKEN`.

This file is the merge-order document. App code lives in the PRs listed in §S4.

---

## S0. Conductor status — 2026-09-21 post-#15 (Julio)

Verified against live GitHub (`juliosuas/ghost`), not the plan-time snapshot.

| PR | Title | Live state | vs `main` |
|---|---|---|---|
| [#14](https://github.com/juliosuas/ghost/pull/14) | plan: ghost hardening (OSS local tool) | OPEN, not draft | MERGEABLE, still based on `22ef1c2`. Plan-only. **Stale.** Close after this update lands. |
| [#15](https://github.com/juliosuas/ghost/pull/15) | feat(doctor): fail on insecure defaults + honesty/CI | **MERGED** 2026-09-21 | `be90513` on `main`. A+B DONE. Do **not** regress `ghost/core/doctor.py` / `tests/test_doctor.py`. |
| [#16](https://github.com/juliosuas/ghost/pull/16) | Harden local Flask defaults (loopback, API token, secrets) | **DRAFT + CONFLICTING** (`DIRTY`) | Branch `cursor/infra-security-defaults-6d9d` @ `831843c`, parent `22ef1c2`. CI **green** on that pre-#15 branch (Python 3.10/3.11/3.12). Gilfoyle owns the product merge. |
...
## S1. HARDENING_PLAN vs current `main` — obsolete vs missing

### Obsolete (done; do not re-implement)

| Plan item | What landed on `main` @ `be90513` |
|---|---|
| PR1 H1 / positioning | README subtitle is "Local authorized OSINT case files + provenance". `pyproject.toml` description matches. CLI banner matches. Zero hits of `AI-Powered OSINT Investigation Platform`. |
| PR1 social/darkweb honesty (docs only) | Feature table marks both ⚠️ experimental / not Quick Start. Quick Start uses `--modules username`. |
| PR1 Quick Start | Copy-paste path is real: `pip install -e .`, `python3 -m ghost doctor`, `investigate … --no-ai --authorized`. |
| PR2 doctor FAIL | `GHOST_SECRET_KEY` / `GHOST_HOST` / `GHOST_API_TOKEN` are error-severity checks. Fresh clone: secret+token FAIL, **unset host OK** (CLI-only). JSON names match locked env vars. Detail never echoes secrets. |
| PR2 tests | `tests/test_doctor.py` covers missing/insecure secret, public vs loopback host, empty token, secure env, JSON/human exit codes, `investigate --help` without secrets. |
| PR2 CI trap | CI does **not** run `ghost doctor` as a job. Unit tests cover both paths. Keep it that way. |
| `GHOST_API_TOKEN` "does not exist" | Field exists on `Config` (`os.getenv("GHOST_API_TOKEN", "")`). **Not** enforced in Flask yet — that is #16. |
| Plan-time "two agents RUNNING, branch null" | Both opened PRs. A+B merged. Auth is #16. |

Accepted divergences from the original PR2 spec (keep; do not "fix" back):

- Unset `GHOST_HOST` is **OK** (CLI-only). Original plan table wanted `error_count >= 3` including host on `Config()` defaults. #15 is the better product: doctor does not block investigate. Tests use explicit `0.0.0.0` for the red host path.
- Secret length `< 16` was in the plan; **#15 does not FAIL short secrets**. Do not expand doctor this week unless Gilfoyle asks. Flask boot (#16) already rejects placeholders.
- Doctor checks live `os.environ`, not the `Config` singleton. That is why unset host ≠ code default `0.0.0.0`.

### Still missing (actionable)

| Gap | Owner | Where | Notes |
|---|---|---|---|
| Flask still open | Gilfoyle #16 | `ghost/backend/server.py` | No token middleware. `CORS(app)` is `*`. `authorized_use` is a JSON boolean, not auth. |
| Bind default still `0.0.0.0` | Gilfoyle #16 | `ghost/core/config.py` L39 | Doctor fails only if the env var is **set** off-loopback. Starting Flask without `GHOST_HOST` still binds all interfaces. |
| Secret still `ghost-dev-key` | Gilfoyle #16 | `config.py` L42 | Doctor fails if env is missing/placeholder; the Flask process still has a known default until #16. |
| `app.secret_key` unused | Gilfoyle #16 | `server.py` | Dead config until assign. |
| Rate limit unused | Gilfoyle #16 | `Config.rate_limit_*` | #16 enforces in-memory on `/api/*`. |
| No `/api/health` | Gilfoyle #16 | `server.py` / compose | Compose healthcheck still `curl` against a missing route. |
| Docker root + 0.0.0.0 publish | Gilfoyle #16 | `Dockerfile`, `docker-compose.yml` | #16: USER 1000, host publish `127.0.0.1`, urllib healthcheck. **Volume path `/app/data` vs code `ghost/data` still unfixed.** |
| README still invites open API | Dinesh leftover PR1 / Gilfoyle 15-line API block | README REST + Docker + config table | Curl example has no token. Config table still says "Flask does not enforce this yet" and "code default is still 0.0.0.0". Docker `docker-compose up -d` is still a happy path. |
| README roadmap still SaaS-shaped | Dinesh leftover PR1 | README "Roadmap" + "Areas We Need Help" | Team Collaboration, Telegram Bot, Mobile App, Scheduled Monitoring, Graph Database, Plugin System still listed. Help still asks for Dashboard UI / new OSINT modules / translations. **Out of Gilfoyle's file lock.** Dinesh follow-up **after** #16, or kill Friday if Express wins. |
| Provenance keys not frozen | Julio PR4 | `ReportGenerator._build_provenance` | Existing tests check a subset. Missing key (`generated_at`, `investigation_id`, `module_count`, `global_errors`) would not fail CI. |
| `social` / `darkweb` still auto-run | Julio/Dinesh PR5 | `INPUT_TYPE_MODULES` | README says experimental; code still injects them for username/email/phone/name. `TestModuleRouting.test_username_modules` **requires** social. |
| Deps drift | Julio PR6 stretch | `pyproject.toml` vs `requirements.txt` | Docker still `pip install -r requirements.txt` (includes `face-recognition`/dlib). CI uses `pip install -e ".[dev]"`. |
| HTML template path | DROP | `ghost/templates/` vs repo-root `templates/` | JSON contract is the week; do not fix. |
| Doctor honesty strings after #16 | Dinesh, **after** #16 merges | `doctor.py` + `cli.py` help | Unset-host detail still says "code default is still 0.0.0.0". Token detail still says "Flask token auth is not wired yet". **Do not edit these in the #16 rebase** (file lock). One-liner follow-up once Flask is wired. |

### Do not regress (#15 file lock)

Gilfoyle / rebase MUST NOT rewrite:

- `ghost/core/doctor.py`
- `tests/test_doctor.py`
- Doctor help in `ghost/ui/cli.py`
- Doctor JSON check **names** (`GHOST_SECRET_KEY`, `GHOST_HOST`, `GHOST_API_TOKEN`)

Reading `Config.api_token` / calling `validate_flask_runtime` in `server.py` is fine. Duplicating `INSECURE_SECRET_DEFAULTS` in `config.py` for boot-time validation is fine (Flask may be stricter than doctor).
...
Both #15 and #16 branched from `22ef1c2`. #15 merged first. Three-way conflict files (verified `git merge-tree`):

| File | Why | Resolution |
|---|---|---|
| `.env.example` | #15 added doctor-oriented GHOST_* block; #16 added a SERVER CONFIG block on the old "OSINT Platform" header | Keep **#15 structure** (CLI-only vs doctor FAIL). Update comments so Flask **does** enforce token/bind. Keep locked names. Keep #16 rate-limit "enforced" note. |
| `CHANGELOG.md` | both prepended `## [Unreleased]` | Keep **all #15 Added/Changed bullets**. Insert #16 `### Security` block above them. |
| `ghost/core/config.py` | #15 added comments + `api_token` field, left host default `0.0.0.0` / secret `ghost-dev-key`; #16 rewrites Server fields to loopback + empty secret + validators | Take **#16 runtime**: `host` default `127.0.0.1`, `secret_key` no shippable default, `validate_flask_runtime`. Do **not** move doctor checks into this file. Leave `enabled_modules` **unchanged** (PR5). |
| `tests/test_investigator.py` | #15 added doctor names to `TestDatabaseConfiguration`; #16 wraps `TestApiSafetyGates` with token headers | Keep **both**: #15 doctor assertions stay; API tests send `Authorization: Bearer` / `X-Ghost-Token` and call `reset_rate_limiter()`. |

Clean auto-merges (take #16): `Dockerfile`, `docker-compose.yml`, `ghost/backend/server.py`, `tests/test_security.py` (new), `tests/test_modules.py` default-config host/secret/token asserts.

### Exact commands (Gilfoyle)

```bash
git fetch origin main
git checkout cursor/infra-security-defaults-6d9d   # or cherry-pick 831843c onto a new branch from main
git rebase origin/main
# resolve the four files using the table above
# NEVER checkout --ours/--theirs on ghost/core/doctor.py (it should not even conflict)
git add .env.example CHANGELOG.md ghost/core/config.py tests/test_investigator.py
git rebase --continue
...
Equivalent if you do not want to rebase the draft in place:

```bash
git checkout -b cursor/auth-rebase-onto-main-f486 origin/main
git cherry-pick 831843c
# same four-file resolution
```

### Files #16 must still not touch

`ghost/core/doctor.py`, `tests/test_doctor.py`, `ghost/ui/cli.py` (doctor help). README: **max ~15 lines** on Local API (token header, loopback, `/api/health`). Do not rewrite Roadmap.

### #16 vs original PR3 spec (keep unless Gilfoyle objects)

| Spec | #16 actual | Call |
|---|---|---|
| Bearer only | also `X-Ghost-Token` | Keep both; doctor/env name unchanged |
| 403 on mismatch | 401 for missing **and** mismatch | Acceptable; tests should not require 403 |
| health `{"ok": true}` | `{"status": "ok"}` | Keep; tests pin `status` |
| Volume `/app/ghost/data` | still `/app/data` vs code `DATA_DIR = <package>/data` | **Still a SHOULD gap.** Do not block Friday MUST/SHOULD on it; log as known leftover |
| Docker `pip install .` | still `requirements.txt` (dlib risk) | PR6. Do not expand #16 |
| CORS off | localhost origins, not `*` | OK |

### Acceptance for the rebased auth PR

- [ ] Merges clean onto `be90513` (or later `main`)
- [ ] `git diff origin/main -- ghost/core/doctor.py tests/test_doctor.py` is empty
- [ ] `python -m ghost.backend.server` without token/secret → does not bind
- [ ] `GET /api/investigations` without header → 401; with Bearer → 200
- [ ] `POST /api/investigate` without `authorized_use: true` → 400 even with token
- [ ] `GET /api/health` → 200, no token
- [ ] No `Access-Control-Allow-Origin: *`
- [ ] Dockerfile `USER` non-root; compose publishes `127.0.0.1`; healthcheck does not use missing `curl`
- [ ] `tests/test_doctor.py` still 100% pass (CLI-only investigate not blocked)
- [ ] Existing `#15` doctor JSON names unchanged

---

## S3. Specs for PRs 4 and 5 (shipped as real diffs)

### PR4 — provenance contract (`tests/test_provenance_contract.py` only)

Freeze `_build_provenance` keys as **set equality**:
...
Fixture: `examples/sample-username-case.json` through `ReportGenerator().generate(..., "json")`.
Do **not** change `report_generator.py`. Do **not** add "confidence per finding".
Do not HTTP / OpenAI / live modules.

### PR5 — experimental modules off-by-default

- `INPUT_TYPE_MODULES["username"|"email"|"name"]` lose `social` and `darkweb`
- `INPUT_TYPE_MODULES["phone"]` loses `social`
- `Config.enabled_modules` default loses `social` and `darkweb`
- Explicit `--modules social` / `--modules darkweb` still runs the registered module (bypass the enabled-list filter when the caller passes a module list)
- Modules are **not** deleted
- Tests in `tests/test_experimental_modules.py` (avoid fighting #16's `TestApiSafetyGates` hunk)
- Update `TestModuleRouting` (it currently **requires** social)

### PR6 — deps (stretch; kill Friday if not in review)

`pyproject.toml` source of truth. Docker `pip install .`. Do not promote `face-recognition` / sqlalchemy / alembic to core.

---

## S4. Ordered PR list through Fri 25 Sep 2026

Owners: **Dinesh** = product merge on Ghost. **Gilfoyle** = auth product. **Julio** = conductor + small PRs.
...
- Next: Ghost waits. No SaaS. No DNS. No Fairway.
```

MUST still = README honesty + doctor FAIL → **already on main**.
SHOULD still = #16 + provenance + experimental off.
DROP = PR6, module-test expansion, HTML template path, dashboard, product rate limiter.

If Express (Tu Web Ideal) needs the team before Friday: merge whatever SHOULD is already green; close the rest; Ghost waits.

---

## Historical plan (written against `main` @ `22ef1c2`)

The sections below are the original PR #14 text. Use §S0–S4 for current merge order. Do not re-do PR1/PR2.
