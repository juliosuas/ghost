...
class GeolocationModule:
    ...
    async def run(self, target: str, input_type: str = "domain") -> dict[str, Any]:
        ...
        results = await asyncio.gather(
            self._ip_geolocation(ip) if ip else asyncio.coroutine(lambda: {"error": "No IP"})(),
            self._shodan_lookup(ip) if ip else asyncio.coroutine(lambda: {"error": "No IP"})(),
        ...
        return {
            "target": target,
            "resolved_ip": ip,
            "geolocation": results[0] if not isinstance(results[0], Exception) else {"error": str(results[0])},
            "shodan": results[1] if not isinstance(results[1], Exception) else {"error": str(results[1])},
        }
